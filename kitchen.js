window.RochePlugin.register({
  id:"char-kitchen", name:"给 Char 炒菜的厨房", version:"4.1.0",
  apps:[{
    id:"char-kitchen-home", name:"Char 的厨房", icon:"restaurant",
    async mount(container, roche){
      ...
      const ACHIEVEMENTS = {
        ...
        ten_dishes:  {icon:"📚", name:"厨神修行",   desc:"菜谱簿累计 10 道菜"},
      ...
      .pan-food span{position:absolute;font-size:18px;transform:translate(-50%,-50%);
        animation:bob 2s ease-in-out infinite;will-change:transform;}
      .pan-food span img{display:block;}
      @keyframes bob{
        0%,100%{transform:translate(-50%,-50%) rotate(0);}
        50%{transform:translate(calc(-50% + var(--jx,0px)),calc(-58% + var(--jy,0px))) rotate(var(--jr,0deg));}
      }
      .pan-holder.toss .pan-food span{animation:jitter .6s ease-in-out;}
      @keyframes jitter{
        0%{transform:translate(-50%,-50%);}
        25%{transform:translate(calc(-50% + var(--jx,6px)),calc(-60% + var(--jy,-4px))) rotate(20deg);}
        50%{transform:translate(calc(-50% + var(--jx2,-8px)),calc(-55% + var(--jy2,3px))) rotate(-15deg);}
        75%{transform:translate(calc(-50% + var(--jx3,4px)),calc(-58% + var(--jy3,-2px))) rotate(10deg);}
      ...
      @keyframes sp-burst{0%{transform:translate(0,0) scale(.3);opacity:1;}
        100%{transform:translate(var(--dx),var(--dy)) scale(1);opacity:0;}}
      @keyframes sp-drip{0%{transform:translate(0,0);opacity:1;}
        100%{transform:translate(var(--dx),var(--dy)) scale(.5);opacity:0;}}
      @keyframes sp-rise{0%{transform:translate(0,0);opacity:0;}
        20%{opacity:1;}100%{transform:translate(var(--dx),calc(var(--dy) - 40px));opacity:0;}}
      @keyframes sp-swirl{0%{transform:translate(0,0) rotate(0);opacity:1;}
        100%{transform:translate(var(--dx),var(--dy)) rotate(720deg);opacity:0;}}
      @keyframes sp-splash{0%{transform:translate(0,0) scale(0);opacity:1;}
      ...
      @keyframes sp-flambe{0%{transform:translate(0,0) scale(.3);opacity:1;}
        50%{transform:translate(calc(var(--dx)/2),calc(var(--dy) - 20px)) scale(1.8);opacity:.9;}
        100%{transform:translate(var(--dx),var(--dy)) scale(2.5);opacity:0;}}
      @keyframes sp-twinkle{0%,100%{transform:translate(0,0) scale(1);opacity:.3;}
        50%{transform:translate(var(--dx),var(--dy)) scale(1.5) rotate(180deg);opacity:1;}}
      @keyframes sp-drop{0%{transform:translate(0,0);opacity:0;}
        30%{opacity:1;}100%{transform:translate(var(--dx),var(--dy)) scale(.8);opacity:0;}}
      @keyframes sp-bubble{0%{transform:translate(0,0) scale(.5);opacity:0;}
        30%{opacity:1;}100%{transform:translate(var(--dx),var(--dy)) scale(1.4);opacity:0;}}
      ...
      .loader-box{background:var(--card);border-radius:20px;padding:32px 44px;
        text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.5),0 0 0 1px rgba(255,255,255,.05);
        min-width:220px;}
      ...
      function playSpiceFx(spice){
        ...
        const oy=rect.top+rect.height*0.35; // 锅口
      ...
      function renderStove(el){
        const foodDots=S.picked.map((p)=>{
          ...
          if(custom){
            emo = custom.image
              ? `<img src="${custom.image}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;box-shadow:0 2px 4px rgba(0,0,0,.3);">`
              : (custom.emoji || "🖼");
          } else emo = p;
          // 只在锅内椭圆随机
          const cx=100+(Math.random()-.5)*100;
          const cy=45+(Math.random()-.5)*20;
          const jx=(Math.random()-.5)*10, jy=(Math.random()-.5)*6, jr=(Math.random()-.5)*30;
          return `<span style="left:${cx}px;top:${cy}px;--jx:${jx}px;--jy:${jy}px;--jr:${jr}deg;--jx2:${-jx}px;--jy2:${-jy}px;--jx3:${jx/2}px;--jy3:${jy/2}px;">${emo}</span>`;
        }).join("");
      ...
      async function cookDish(){
        ...
        S.picked.forEach(e=>{
          if(e.startsWith("::")){
            ...
            parts.push(e); const m=EMOJI_META[e];
        ...
        if(!dupe){
          ...
          if(rec.dark) await unlock("dark_master");
          if(rec.midnight) await unlock("midnight");
          ...
          if(S.cravingBanner) await unlock("craving_ans");
        }
        hideLoading();
      ...
      function renderBook(el){
        ...
        el.querySelectorAll("[data-act]").forEach(b=>b.onclick=async()=>{
          ...
          if(a==="del"){
            ...
            await roche.storage.set("recipes",S.recipes); render();
      ...
      function renderSet(el){
        ...
        <div class="h"><span>🏆 成就（${Object.keys(S.achievements).length}/${Object.keys(ACHIEVEMENTS).length}）</span></div>
        <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(96px,1fr));gap:8px;">
          ${Object.entries(ACHIEVEMENTS).map(([id,a])=>{
            const got=S.achievements[id];
            return `<div class="card" style="text-align:center;padding:10px 6px;margin:0;${got?"":"opacity:.35;filter:grayscale(1);"}">
              <div style="font-size:28px;">${got?a.icon:"🔒"}</div>
              <div style="font-size:12px;font-weight:700;margin-top:4px;">${a.name}</div>
              <div style="font-size:10px;color:#888;line-height:1.3;">${a.desc}</div>
            </div>`;
          }).join("")}
        </div>
        ...
        el.querySelector("#wipeAll").onclick=async()=>{
          ...
          S.recipes=[];S.feeds=[];S.custom=[];S.achievements={};S.spiceUsed=new Set();S.potUsed=new Set();
