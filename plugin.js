  ...
  apps: [{
    ...
    async mount(container, roche) {
      ...
      const ACHIEVEMENTS = {
        ...
        spice_common_all: { icon: "🧂", name: "厨台霸主", desc: "台面调料都用过" },
        spice_all: { icon: "🧪", name: "调料收藏家", desc: "用过全部调料" },
        no_spice: { icon: "🥗", name: "极简主义", desc: "不放任何调料炒一道菜" },
        max_fire: { icon: "🔥", name: "猛火大师", desc: "用猛火出锅一次" },
        low_fire: { icon: "🕯", name: "文火慢炖", desc: "用小火出锅一次" },
        toss_master: { icon: "🥢", name: "颠勺达人", desc: "累计颠勺或搅拌 20 次" },
        toss_hundred: { icon: "💫", name: "颠勺宗师", desc: "累计颠勺或搅拌 100 次" },
        ...
        spice_junkie: { icon: "⚗️", name: "调料狂魔", desc: "单次使用 5 次以上调料" },
        ...
        all_dark_emojis: { icon: "👺", name: "深渊图鉴", desc: "3 种怪东西全部用过" },
      ...
      const S = {
        ...
        achOpen: true, konamiSeq: [], stirTimer: null
      };
      ...
      container.innerHTML = `
        <div class="ck" data-theme="${S.theme}">
          ...
          <div class="ck-nav" id="ckNav">
            ${[["stove", ICONS.stove, "料理台"], ["book", ICONS.book, "菜谱"], ["feed", ICONS.feed, "投喂"], ["custom", ICONS.custom, "自定义"], ["set", ICONS.set, "设置"]]
      ...
      function playSpiceFx(spice) {
        ...
        if (!layer) { layer = document.createElement("div"); layer.className = "spice-fx-layer"; root.appendChild(layer); }
      ...
      function darkFlash() {
        ...
        root.appendChild(f); setTimeout(() => f.remove(), 1700);
        root.style.animation = "bodyShake .1s linear 8"; setTimeout(() => { root.style.animation = ""; }, 900);
      }
      ...
      function renderStove(el) {
        ...
        el.innerHTML = `
          ...
          <div class="grid">${SPICES_COMMON.map(s => `<div class="cell" data-sp="${s}" title="${SPICE_FX[s]?.name || ""}">${s}</div>`).join("")}</div>
          <div class="cabinet">
          ...
          <div style="margin-top:16px;display:flex;gap:8px;flex-wrap:wrap;">
            <button class="btn" id="toss" ${S.picked.length === 0 || S.fire === 0 ? "disabled" : ""}>${actionBtnText}</button>
            <button class="btn" id="cook" ${!canCook ? "disabled" : ""}>${cookBtnText}</button>
            <button class="btn ghost" id="clr">重置厨房</button>
          </div>`;
        ...
        $("#fire").oninput = (e) => { 
          ...
          $("#flame").outerHTML = `<div class="flame-holder" id="flame" style="top:${90 - S.fire * 5}px">${flameSVG(S.fire)}</div>`; 
        ...
        el.querySelectorAll("[data-sp]").forEach(x => x.onclick = () => { 
          ...
          S.spices.push(s); 
          render(); 
          setTimeout(() => playSpiceFx(s), 30); 
        });
        if ($("#clrSpices")) $("#clrSpices").onclick = () => { S.spices = []; render(); };
        ...
        $("#toss").onclick = async () => {
          if (S.tool === "spatula") {
            ...
            const tIcon = $("#toolIcon"); tIcon.classList.remove("toss"); void tIcon.offsetWidth; tIcon.classList.add("toss");
            let n = 0; 
            const t = setInterval(() => { 
            ...
            const tIcon = $("#toolIcon");
            if (tIcon.classList.contains("stir")) {
              tIcon.classList.remove("stir");
              clearInterval(S.stirTimer);
            } else {
              tIcon.classList.add("stir");
              let angle = 0;
              S.stirTimer = setInterval(() => {
                angle += 10;
                container.querySelectorAll(".pan-food span").forEach((s, i) => {
                  const r = 30 + (i % 3) * 10;
                  const a = angle + i * (360 / Math.max(1, S.picked.length));
                  const rad = a * Math.PI / 180;
                  s.style.left = (100 + Math.cos(rad) * r) + "px";
                  s.style.top = (45 + Math.sin(rad) * (r * 0.3)) + "px";
                });
              }, 50);
              setTimeout(() => {
                if(tIcon) tIcon.classList.remove("stir");
                clearInterval(S.stirTimer);
              }, 1500);
            }
          }
      ...
      async function cookDish() {
        ...
        if (dark) {
          const darkHints = [
            "表面看起来精致好看，实则味道令人作呕、极度难吃。请起一个反差感强或欺骗性的菜名，描述其美丽的伪装和恶心的味道。",
            "外观一言难尽、极度丑陋恶心，但吃起来却意外地美味绝伦。请描述这种极端的反差。",
            "完全是一坨焦黑的不明物质，散发着糊味。请起一个带有毁灭感或焦炭相关的菜名，描述其毁灭性的外观和口感。",
            "呈现出诡异的糊状物或黏液状态，甚至可能在冒泡或蠕动。请起一个克苏鲁风或生化危机风的菜名，描述其令人不安的质地。"
          ];
          const selectedDarkHint = darkHints[Math.floor(Math.random() * darkHints.length)];
          darkHint = `\n【重要】这是一道黑暗料理！设定为：${selectedDarkHint} 卖相、功效请完全符合此设定，要有创意。`;
        }

        const counts = {}; S.spices.forEach(s => counts[s] = (counts[s] || 0) + 1);
        const overSpiced = Object.entries(counts).filter(([s, c]) => c >= 3).map(([s]) => SPICE_FX[s]?.name || s);
        const spiceHint = overSpiced.length > 0 ? `\n【提示】由于${overSpiced.join("、")}加了太多了，口感会受严重影响，描述时请务必体现这一点。` : "";

        const sysPrompt = isColdPrep
          ? `你是会取名的甜点师/调酒师。根据食材和调料，为这道【免火冷调】的甜点/饮品/冷盘起中文名。JSON：{"name":"","desc":"","effect":"","appearance":""}${midnightHint}${darkHint}${spiceHint}`
          : `你是会取名的厨师。根据食材、调料、锅具、火候起中文菜名。JSON：{"name":"","desc":"","effect":"","appearance":""}${midnightHint}${darkHint}${spiceHint}`;
        ...
        if (!dupe) {
          ...
          if (S.darkEmojisUsed.size >= 3) await unlock("all_dark_emojis");
        ...
        if (S.feedTab === "feedChar") {
          ...
          area.innerHTML = `
            ...
            ${chars.length ? chars.map(c => `
              ...
              </div>`).join("") : `<div style="text-align:center;color:#aaa;padding:20px;">没有 Char</div>`}`;
...
<br>Char 随时可能端着食物来找你。<br>点击飘浮的泡泡看看是谁带了什么好吃的！
        ...
        const timer = setInterval(() => {
          ...
          b.style.cssText = `
            position: absolute; left: ${10 + Math.random() * 70}%;
            font-size: 48px; padding: 20px; cursor: pointer; pointer-events: auto;
            transition: transform 0.2s; animation: slowFloatUp ${10 + Math.random() * 6}s linear forwards;
      ...
      function renderPendingFeed(el) {
        ...
        el.innerHTML = `
          <div style="padding:20px; display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; text-align:center; animation: fadeIn 0.4s;">
            ...
            <div style="display:flex; gap:12px; width:100%; max-width:300px;">
              ...
              <button class="btn ghost" id="btnTasteFeed" style="flex:1;">尝一口</button>
              <button class="btn" id="btnAcceptFeed" style="flex:1;">吃掉！</button>
            </div>
          </div>
        `;
        ...
        const startChat = async (action) => {
          ...
          S.chatWith = { char, dish, isPureChat: true, userAction: action };
      ...
      async function feedToChar(dish, char) {
        ...
        S.chatLog = [{ role: "assistant", text: payload.spoken || payload.feeling }];
        ...
        el.innerHTML = `
          <div class="chat-page">
            ...
            <div class="status-grid" style="max-height:${S.chatStatusOpen ? "200px" : "0"}; opacity:${S.chatStatusOpen ? "1" : "0"}; margin-bottom:${S.chatStatusOpen ? "10px" : "0"};">
              ...
              <div class="status-cell" style="grid-column:span 2;"><div class="lbl">👅 真实感受</div>${status.feeling || "—"}</div>
            </div>
